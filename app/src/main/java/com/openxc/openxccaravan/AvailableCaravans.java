        ...
        AvailableCaravans.this.runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (message.asNamedMessage().getName().equals("caravan_data")) {
                    if (message.asSimpleMessage().getValue().equals("caravans")) {
                    ...
                    else if (message.asSimpleMessage().getValue().equals("join_resp")) {
                        // Display the proper pop-up message
                        if (message.getExtras().get("result").equals("GRANTED")) {
                            DialogFragment join_success = new JoinSuccessDialog();
                            join_success.show(getFragmentManager(),"granted");
                        }
                        else {
                            DialogFragment join_fail = new JoinFailedDialog();
                            Bundle args = new Bundle();
                            args.putString("reason",message.getExtras().get("reason").toString());
                            join_fail.setArguments(args);
                            join_fail.show(getFragmentManager(),"denied");
                        }
                    }
                }
            }
